...
objUid = p.loadURDF(os.path.join(os.path.dirname(__file__),"youbot_description/youbot.urdf"), startPos, startOrientation)
...
tableUid = p.loadURDF("table/table.urdf", basePosition = [0.6,0,0.0], globalScaling = 0.3)
jengaUid = p.loadURDF("jenga/jenga.urdf",[0.7,0,0.2], globalScaling = 0.6)
p.changeVisualShape(jengaUid,-1,rgbaColor=[0.58,0.29,0,1])
duckUid = p.loadURDF("duck_vhacd.urdf", basePosition = [0.5,-0.1,0.2], globalScaling = 0.75)
p.changeDynamics(duckUid,-1,linearDamping=1, angularDamping=1, rollingFriction=0.1, spinningFriction=0.1)

mugUid = p.loadURDF("objects/mug.urdf", basePosition = [0.5,0.1,0.2], globalScaling = 0.5)
bikeOrn = p.getQuaternionFromEuler([1.57,0,0])
bikeUid = p.loadURDF("bicycle/bike.urdf", basePosition = [1, 0.3, 0.1], baseOrientation = bikeOrn, globalScaling = 0.25)
...
while True:
	...
	for _id in range(p.getNumJoints(objUid)):
		...
		if(joint_pos in [0,1,2]):
			maxForceArr.append(1e6)
		else:
